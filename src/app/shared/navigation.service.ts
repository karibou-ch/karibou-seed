...
export class KngNavigationStateService  {
  ...
  updateUser(user){
    ...
    this.isShopOwner=(user.shops.length||user.isAdmin());
    this.isLogistic=(user.hasRole('logistic')||user.isAdmin());
    this.isAuthenticated=user.isAuthenticated();
    // console.log('----user',user.display())
    // console.log('----isAdmin',this.isAdmin)
    // console.log('----isShopOwner',this.isShopOwner)
    // console.log('----isLogistic',this.isLogistic)
  ...
  getRouterLink(url){
    if(typeof url!=='string'){
      return ['/store',this.store].concat(url);
    }
  ...
  // 
  // return all grouped menuItems
  getMenus(){
    // return Object.keys(this.menu);
    return ["howto","karibou"];
  }


  dispatch(url:any[],params:any){
    return this.content;
  }
