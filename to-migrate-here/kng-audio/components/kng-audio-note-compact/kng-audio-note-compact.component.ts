...
export class KngAudioNoteCompactComponent implements OnInit, OnDestroy {
  ...
  @Input() type: AudioNoteType = 'prompt';
  ...
  private async uploadAudio(blob: Blob, base64: string, duration: number) {
    try {
      // ✅ Utiliser /api/transcribe directement
      const formData = new FormData();
      const filename = this.filename || `${this.type}-compact-${Date.now()}.wav`;
      ...
      // Émettre résultat avec transcription
      this.onAudioReady.emit({
        src: audioUrl,
        audio: base64,
        note: result.transcription || '',
        duration
      });
